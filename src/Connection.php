...
{
    /**
     * @var boolean Use pconnect
     */
    public $use_pconnect = false;

    /**
     * @var Redis resource redis object
     */
    ...
    public function open()
    {
        ...
        try {
            ...
            if ($this->use_pconnect) {
                $this->_redis->pconnect($this->hostname, $this->port, $this->connectionTimeout, null, 0, $this->dataTimeout);
            } else {
                $this->_redis->connect($this->hostname, $this->port, $this->connectionTimeout, null, 0, $this->dataTimeout);
            }
            $this->_redis->setOption(Redis::OPT_REPLY_LITERAL, true);
    ...
    protected function parseRequest($params)
    {
        foreach ($params as $key => $value) {
            if (is_array($value)) {
                foreach (self::parseRequest($value) as $item) {
                    yield $item;
                };
            } elseif ($value === null) {
                continue;
